...
class FastE2ETest : public ::testing::Test {
 ...
 protected:
  virtual void SetUp() {
    ...
    for (size_t sphere_idx = 0u; sphere_idx < kNumDifferentSpheres;
      ...
      colors_vector_[sphere_idx].resize(
          sphere_points_C_vector_[sphere_idx].size(),
          voxblox::Color(128, 255, 0));
    ...
    baseline_mesh_layer_.reset(
        new voxblox::MeshLayer(kVoxelSize * kVoxelsPerSide));
    baseline_mesh_integrator_.reset(new voxblox::MeshIntegrator(
        baseline_config, baseline_layer_.get(), baseline_mesh_layer_.get()));

    fast_mesh_layer_.reset(
        new voxblox_fast::MeshLayer(kVoxelSize * kVoxelsPerSide));
    fast_mesh_integrator_.reset(new voxblox_fast::MeshIntegrator(
        fast_config, fast_layer_.get(), fast_mesh_layer_.get()));
  }
  ...
  htwfsc_benchmarks::test::LayerTest<voxblox::TsdfVoxel,
                                     voxblox_fast::TsdfVoxel>
      layer_test_;
